...
### GDSC DATASET
gdsc_e <- read_csv("exacloud_folder/gdsc_e_aml_kegg.csv", col_names = TRUE)
gdsc_dr <- read_csv("exacloud_folder/gdsc_pancancer_commondrugs_ic50.csv", col_names = TRUE)
gdsc_mut <- read_csv("exacloud_folder/gdsc_mut_aml_kegg.csv", col_names = TRUE)
#gdsc_cp <- read_csv("cancer_systems/gdsc_aml_cp.csv", col_names = TRUE)
...
gdsc_mut_complete$rn <- toupper(gsub("[.]","",gdsc_mut_complete$rn))
gdsc_mut_complete$rn <- toupper(gsub("-","",gdsc_mut_complete$rn))
gdsc_mut_complete$rn <- toupper(gsub(" ","",gdsc_mut_complete$rn))
gdsc_mut_complete$rn <- toupper(gsub("[()]","",gdsc_mut_complete$rn))
...
common_rows <- as.data.frame(Reduce(intersect, list(gdsc_e_complete$RN, gdsc_mut_complete$RN, gdsc_dr_complete$RN)))
...
common_filtered_mut <- left_join(common_rows, gdsc_mut_complete, by="rn")
#list(common_rows, gdsc_mut_complete) %>% reduce(left_join, by="rn")
...
gdsc_features_mut <- data_scaling(as.data.frame(common_filtered_mut))
rownames(gdsc_features_mut) <- as.matrix(common_rows)
gdsc_mut_complete <- gdsc_features_mut[, colSums(is.na(gdsc_features_mut))==0]

#gdsc_features_cp <- data_scaling(as.data.frame(common_filtered_cp))
#rownames(gdsc_features_cp) <- as.matrix(common_rows)
#gdsc_cp_complete <- gdsc_features_cp[, colSums(is.na(gdsc_features_cp))==0]
...
opts <- getDefaultOpts()
DATANAME <- "e_h__ic50_gdsc_24_aml_kegg"
K <- 24
...
if (sum(is.na(gdsc_drug_complete)) == 0 &&
    ...
    sum(is.na(gdsc_mut_complete)) == 0 && sum(is.nan(gdsc_mut_complete)) == 0)
{
  print("Model Learning...")
  model <- GBGFAexperiment(list(gdsc_exp_complete, gdsc_mut_complete, gdsc_drug_complete), 24, opts)
}
